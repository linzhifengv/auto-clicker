...
一个 Android 自动连点器应用，通过无障碍服务实现自动化点击操作。主要用于自动化重复性的屏幕点击任务。

## 功能特性

- **坐标点击**：手动设置点击坐标，精确点击目标位置
- **文本查找点击**：通过屏幕文本自动查找并点击按钮
- **倒计时监控**：自动检测屏幕上的倒计时文本，等待倒计时结束后执行操作
- **悬浮窗控制**：悬浮窗显示实时状态、倒计时读秒，支持开始/暂停/停止
- **坐标录制**：在悬浮窗中直接录制屏幕坐标，设置后自动保存
- **预设模板**：内置汽水音乐领取奖励、通用广告观看等模板
...
1. 打开应用，授予无障碍服务权限和悬浮窗权限
2. 打开目标应用（如汽水音乐），导航到需要自动点击的页面
3. 在悬浮窗中点击「设坐标:领取」，然后在屏幕上点击"领取"按钮的位置
4. 点击「设坐标:奖励」，然后在屏幕上点击"奖励"按钮的位置
5. 坐标会自动保存，下次无需重新设置
